...
impl<'a> Transformer<'a> {
    ...
    pub fn without_self_change(mut self) -> Self {
        log::debug!("create a copy of {:?} without self change", self.src_file);
        ...
        log::debug!("copy done {:?}", self.src_file);
        self
    }
    ...
    pub fn transform_to_fuzzer(
        ...
        log::debug!("Change {:?} to fuzzer, {:?}", self.src_file.file_name(), nths);
    ...
    pub fn transform_with_execution_check(
        ...
        log::debug!("transform_with_execution_check1");
        let transform_len: usize = self.get_transform_len(constraints)?;
        log::debug!("transform_with_execution_check2");
        let mut changable_args = Vec::new();
        let mut executor = Executor::new(self.deopt)?;
        log::debug!("transform_with_execution_check3");
        for nth in 0..transform_len {
            log::debug!("transform_with_execution_check4");
            ...
            if transformer.check_must_transformed_arg(constraints, nth)? {
                ...
                log::debug!("transform_with_execution_check4-1");
                continue;
            }
            log::debug!("transform_with_execution_check4-2");
            transformer.transform_to_fuzzer(constraints, vec![nth])?;
            log::debug!("transform_with_execution_check4-2 2");
            let out_driver = transformer.get_output_file();
            log::debug!("transform_with_execution_check4-2 3");
        ...
        log::debug!("transform_with_execution_check5");
        Ok(changable_args)
    }
    ...
    pub fn check_must_transformed_arg(&mut self, constraints: &APIConstraints, nth: usize) -> Result<bool> {
        ...
        for fuzz_variant in &fuzz_variants {
            ...
            log::debug!("check must transformed arg: {nth}");
            log::debug!("call_name: {:?}", fuzz_variant.call_name);
            log::debug!("arg_pos: {:?}", fuzz_variant.arg_pos);
            log::debug!("constraint: {:?}", fuzz_variant.constraint);
            // TODO: check logic
