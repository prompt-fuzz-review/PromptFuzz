...
pub fn infer_constraints(
    ...
    for call in api_calls {
        ...
        for pos in integer_pos {
            ...
            // if utils::is_must_transform(integer_arg, &visitor) {
            //     let constraint = Constraint::LoopCount(pos);
            //     add_function_constraint(&call_name, constraint, constraints);
            //     continue;
            // }
        }
...
mod utils {
    ...
    // pub fn is_must_transform(arg: &Node, visitor: &Visitor) -> bool {
    //     let arg = strip_prefix(arg);
    //     let must_transform = ["flag", "mode", "Flag"];
    //     if let Clang::CallExpr(ce) = &arg.kind {
    //         let name = ce.get_name_as_string(arg);
    //         log::debug!("is_must_transform(name: {:?})", name);
    //         for mt in must_transform {
    //             if name.contains(mt) {
    //                 return true;
    //             }
    ...
    //     if let Clang::DeclRefExpr(dre) = &arg.kind {
    //         if let Some(init) = dre.get_var_init(visitor) {
    //             return is_must_transform(init, visitor);
    //         }
    //     }
    //     false
    // }
