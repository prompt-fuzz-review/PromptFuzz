...
pub fn transform(
    ...
    log::debug!("TEST: transfrom(project: {:?})", project);
...
fn fuse_fuzzer(
    ...
    log::debug!("TEST: project: {:?}", project);
    log::debug!("TEST: seed_dir: {:?}", seed_dir);
    log::debug!("TEST: n_fuzzer: {:?}", n_fuzzer);
    log::debug!("TEST: core: {:?}", core);
    log::debug!("TEST: use_cons: {:?}", use_cons);
    let deopt = Deopt::new(project)?;
